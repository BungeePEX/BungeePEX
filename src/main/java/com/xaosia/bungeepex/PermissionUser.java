...
@ToString
@EqualsAndHashCode
public class PermissionUser implements PermEntity {
    ...
    public PermissionUser(String name, UUID UUID, List<PermissionGroup> groups, List<String> extraPerms, Map<String, Server> servers, String display, String prefix, String suffix)
    {
    ...
    public boolean hasPerm(Sender s, String perm)
    {
        ...
        //pre process
        perms = BungeePEX.getInstance().getPermissionsResolver().preprocess(perms, s);

        //resolve
        Boolean has = BungeePEX.getInstance().getPermissionsResolver().has(perms, perm);

        //post process
        has = BungeePEX.getInstance().getPermissionsResolver().postprocess(perm, has, s);
    ...
    public void recalcPerms()
    {
        ...
        //call event
        BungeePEX.getInstance().getEventDispatcher().dispatchUserChangeEvent(this);
    }
    ...
    public PermissionGroup getGroupByLadder(String ladder)
    {
        access();
    ...
    public List<Privilege> getPermsWithOrigin(String server, String world)
    {
        ...
        for (String s : perms)
        {
            Privilege perm = new Privilege(s, name, false, null, null);
            ret.add(perm);
        }
        ...
        for (Map.Entry<String, Server> srv : servers.entrySet())
        {
            ...
            {
                Privilege perm = new Privilege(s, name, false, srv.getKey(), null);
                ret.add(perm);
            }
            ...
            for (Map.Entry<String, World> w : srv.getValue().getWorlds().entrySet())
            {
                ...
                {
                    Privilege perm2 = new Privilege(s2, name, false, srv.getKey(), w.getKey());
                    ret.add(perm2);
                }
            }
        }
        ...
        //pre process
        ret = BungeePEX.getInstance().getPermissionsResolver().preprocessWithOrigin(ret, getSender());
    ...
    public String buildPrefix(String server, String world)
    {
        ...
        return prefix
                + (BungeePEX.getInstance().getConfig().isTerminatePrefixSpace() ? " " : "")
                + (BungeePEX.getInstance().getConfig().isTerminatePrefixReset() ? ChatColor.RESET : "");
    }
    ...
    public String buildSuffix(String server, String world)
    {
        ...
        return suffix
                + (BungeePEX.getInstance().getConfig().isTerminateSuffixSpace() ? " " : "")
                + (BungeePEX.getInstance().getConfig().isTerminateSuffixReset() ? ChatColor.RESET : "");
    }
    ...
    {
        return BungeePEX.getInstance().getConfig().isUseUUIDs()
                ? BungeePEX.getInstance().getPlugin().getPlayer(UUID)
                : BungeePEX.getInstance().getPlugin().getPlayer(name);
    }
    ...
    private void debug(String perm, boolean result)
    {
        ...
        {
            BungeePEX.getLogger().info("perm check: " + name + " has " + perm + ": " + result);
        }
    }
