...
import com.xaosia.bungeepex.platform.bukkit.bridge.bridges.essentials.EssentialsBridge;
import com.xaosia.bungeepex.platform.bukkit.bridge.bridges.vault.VaultBridge;
import com.xaosia.bungeepex.platform.bukkit.bridge.bridges.worldedit.WorldEditBridge;
...
public class BridgeManager implements Listener
{
    ...
    {
        if (cmd.equalsIgnoreCase("bungeepexbukkitbridge"))
        {
            if (!(sender.isConsole()))
            {
    ...
    {
        if (e.getPlugin().getName().equalsIgnoreCase("BungeePEXBukkitBridge"))
        {
            Bukkit.getConsoleSender().sendMessage(Color.RED + "WARNING: Please remove BungeePEXBukkitBridge!!! It's now integrated in BungeePEXBukkit.");
        }
    }
}
