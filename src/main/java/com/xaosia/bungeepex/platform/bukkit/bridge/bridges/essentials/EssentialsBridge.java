...
public class EssentialsBridge implements Bridge
{
    ...
    {
        BungeePEX.getLogger().info("Injection of BungeePEX into Essentials");
        try
        {
            ...
            BungeePEX bpexPlugin = BungeePEX.getInstance();
            if (bpexPlugin != null && bpexPlugin.isEnabled())
            {
                if (!(handler instanceof BungeePermsHandler))
                {
                    Logger.getLogger("Essentials").log(Level.INFO, "Essentials: Using BungeePEX based permissions.");
    ...
    {
        BungeePEX.getLogger().info("Uninjection of BungeePEX into Essentials");
